...
public static class CurrentSimulationTime
{
    public static DateTimeOffset? GetCurrentSimulationTime()
    {
        ...
        DateTimeOffset simulationDateTimeOffset = new DateTimeOffset(now.Year, now.Month, now.Day, simulationHours,
            simulationMinutes, 0, now.Offset);

        // Return the simulation time
        return simulationDateTimeOffset.ToUniversalTime();
    }
}